...
└─ summary → PDF
PostgreSQL (run state) · Qdrant (vectors) · Ollama (local LLM) · Langfuse + OpenTelemetry (observability)
...
Deterministic harness (`scripts/evaluate.py`) over a golden query set against the indexed 10-K:

| Metric | Value |
|---|---|
| recall@5 | 100.00%|
| MRR | 1.000 |
| Citation groundedness | 100% (every cited excerpt verified as a real corpus substring) |

No LLM judge required — both metrics are pure functions, so they run in CI at zero cost.